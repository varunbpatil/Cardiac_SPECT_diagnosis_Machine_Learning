...
\usepackage[left=0.8in,top=2.7in,right=1in,bottom=1.5in]{geometry} % geometry package is used to easily set all the margins
\usepackage{graphicx}
\usepackage{tabularx}
...
%    \draw[line width=0.5mm] ([xshift=2.7cm,yshift=-1.3cm]current page.north west) rectangle ([xshift=-1.6cm,yshift=10.3cm]current page.south east);
% now drawing the inner line
%    \draw[line width=0.2mm] ([xshift=3cm,yshift=-1.6cm]current page.north west) rectangle ([xshift=-1.9cm,yshift=10.6cm]current page.south east);
%\end{tikzpicture}
